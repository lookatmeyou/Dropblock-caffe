...
void Solver<Dtype>::TestClassification(const int test_net_id) {
  ...
  vector<const Blob<Dtype>* > per_class_result;
  vector<string> per_class_result_names;
  vector<shared_ptr<Blob<Dtype> > > per_class_score;
  for (int i = 0; i < blob_names.size(); ++i) {
    if (blob_names[i].find("AccuracyPerClass") != string::npos) {
      per_class_result_names.push_back(blob_names[i]);
      const Blob<Dtype>* p_blob = test_net->blob_by_name(blob_names[i]).get();
      per_class_result.push_back(p_blob);
      shared_ptr<Blob<Dtype> > blob_pointer(new Blob<Dtype>());
      per_class_score.push_back(blob_pointer);
      per_class_score.back()->ReshapeLike(*p_blob);
      memset(per_class_score.back()->mutable_cpu_data(), 0, sizeof(Dtype) * p_blob->count());
    }
  }
  ...
  for (int i = 0; i < param_.test_iter(test_net_id); ++i) {
    ...
    if (param_.show_per_class_result()) {
      for (int i = 0; i < per_class_score.size(); ++i) {
        for (int j = 0; j < per_class_score[i]->count(); ++j) {
          per_class_score[i]->mutable_cpu_data()[j] += per_class_result[i]->cpu_data()[j];
        }
      }
    }
  }
  ...
  if (param_.show_per_class_result()) {
    for (int i = 0; i < per_class_score.size(); ++i) {
      LOG(INFO) << "    Test net output #" << test_score.size() << ": " << per_class_result_names[i];
      int labels = per_class_score[i]->shape(1);
      for (int j = 0; j < labels; ++j) {
        Dtype mean_score = per_class_score[i]->cpu_data()[j] / per_class_score[i]->cpu_data()[labels + j];
        LOG(INFO) << "        label " << j << ": " << mean_score;
      }
    }
  }
}
